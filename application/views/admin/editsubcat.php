...
<div id="page-wrapper">
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">Edit Sub Catergory</h1>
        ...
        <div class="row">
            <div class="col-lg-6">
                ...
                <form enctype="multipart/form-data" action="<?php echo base_url() . 'admin/EditSubCat/' . $subcatid; ?>" method="post" onsubmit="return validateForm()" name="myform"EditMainCat id="register_form input">
                    <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
                    <?php foreach ($singlesubcat as $cat) { ?>

                        <div class="form-group">
                            <p>Select Parent Category <select name="pcat" class="form-control">
                                    <option value="">--Select Parent Category--</option>
                                    <?php
                                    foreach ($cats as $cats) {
                                        if ($cats->mcatid == $cat->parentcat) {
                                            echo '<option value="' . $cats->mcatid . '" selected="selected">' . $cats->name . '</option>';
                                        } else {
                                            echo '<option value="' . $cats->mcatid . '">' . $cats->name . '</option>';
                                        }
                                        ?>
                                    <?php } ?>
                        ...
                        <button type="submit" class="btn btn-lg btn-ml-login btn-block" name="submit">Update Sub Category</button>
                        <br>
                        <br>
                    </form>
                </div>
...
<script type="text/javascript">
    ...
    function validateForm() {
        ...
        if (pcat == "") {
            $("span.val_pcat").html("This field is required.").addClass('validate');
            validation_holder = 1;
        } else {
            $("span.val_pcat").html("");
        }
